...
1. **File split per domain.** Each domain under `tests/functional/<domain>/[<feature>/]`
   ...
   **Independently-testable sub-features get a subfolder** — `auth/login/`, `auth/logout/`,
   ...
   `AuthController`. `tenant/rate-limit/` and `tenant/audit-log/` (A1) are the same call for
   `TenantsController`: the override and the audit trail are each reached and tested
   independently of the base `GET`/`PATCH /tenant` and of each other, not states of the same
   journey — this applies per-endpoint, not just once per controller, so a controller with three
   independently-tested capabilities gets three subfolders, not one. This is a deliberate
   per-domain call, not a rule that every multi-route controller needs subfolders — a controller
   whose routes are only ever exercised together stays flat (`memberships/`, with its
   create/list/edit/delete all flat under one folder, is the counter-example).

   A subfolder's own file set drops the domain prefix the same way `auth/login/` does
   (`login-api.spec.ts`, not `auth-login-api.spec.ts`) — `rate-limit-override-set-api.spec.ts`, not
   `tenant-rate-limit-override-set-api.spec.ts`; `sendAuditLogRequest` in `audit-log/audit-log-api.actions.ts`,
   not `sendTenantAuditLogRequest`. The remaining `<resource>-<verb>` base
