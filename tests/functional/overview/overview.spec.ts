...
import { acmeTenantDetails } from '../tenant/tenant-api.data';
import { acmeUsageDetails } from './overview.data';
...
test.describe('overview', { tag: ['@overview', '@ui'] }, () => {
  ...
  test('validate owner user sees their tenant summary on the overview page', async ({ page }) => {
    ...
    await assertTenantSummaryVisible(page, acmeTenantDetails, acmeUsageDetails);
