...
import { assertMembershipsListSuccess } from './memberships-api.assertions';
import { acmeMembers, northwindMembers } from './memberships-api.data';

test.describe('memberships api - list', { tag: ['@memberships', '@api'] }, () => {
  ...
  test('validate owner user can list members', async ({ request }) => {
    ...
    const response = await sendMembershipsListRequest(request, ownerToken, getTenantId('acme'));
    await assertMembershipsListSuccess(response, acmeMembers);
  });

  test('validate admin user can list members', async ({ request }) => {
    const adminToken = await generateAccessToken(request, adminLoginBody);
    const response = await sendMembershipsListRequest(request, adminToken, getTenantId('acme'));
    await assertMembershipsListSuccess(response, acmeMembers);
  });
  ...
  test('validate member user can list members', async ({ request }) => {
    ...
    const response = await sendMembershipsListRequest(request, memberToken, getTenantId('acme'));
    await assertMembershipsListSuccess(response, acmeMembers);
  });
  ...
  test('validate viewer user can list members', async ({ request }) => {
    ...
    const response = await sendMembershipsListRequest(request, viewerToken, getTenantId('acme'));
    await assertMembershipsListSuccess(response, acmeMembers);
  });
  ...
  test('validate billing user can list members', async ({ request }) => {
    ...
    const response = await sendMembershipsListRequest(request, billingToken, getTenantId('acme'));
    await assertMembershipsListSuccess(response, acmeMembers);
  });

  test('validate member user can list members under different tenant id it belongs to', async ({ request }) => {
    const memberToken = await generateAccessToken(request, memberLoginBody);
    const response = await sendMembershipsListRequest(request, memberToken, getTenantId('northwind'));
    await assertMembershipsListSuccess(response, northwindMembers);
  });
