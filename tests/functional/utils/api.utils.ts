...
// disposes it after, so any flow function (login, createTenant, revokeKey, ...) can run
// in a hook without a bespoke *InHook variant of its own.
export async function withHookRequestContext<T>(
  playwright: PlaywrightWorkerArgs['playwright'],
  fn: (request: APIRequestContext) => Promise<T>,
): Promise<T> {
  const context = await playwright.request.newContext({ baseURL: environment.apiBaseUrl });
  try {
