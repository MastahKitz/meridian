// Renders the GitHub Pages dashboard's index.html, published by
// build-report-dashboard.js after every Playwright run. Ported from the
// sibling playwright-fullstack-test-framework repo's scripts/lib/render-dashboard.js
// — trimmed to results + trend only, no AI-triage section (that repo's version
// also renders one, fed by a separate qa-triage workflow Meridian doesn't have).
...
function renderHtml({ runs: runList, repoUrl = '', repository = '' }) {
...
<title>Meridian Test Dashboard</title>
<style>
  :root {
...
<body>
<main>
  <h1>Meridian Test Dashboard</h1>
  <p class="sub">
    <a href="${repoUrl || '#'}">${repository || 'meridian'}</a>
